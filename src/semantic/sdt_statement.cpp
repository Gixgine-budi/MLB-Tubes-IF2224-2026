...
Ptr<ast::ExprNode> buildTermLocal(SDTBuilder& builder,
  ...
  for (size_t i = 1; i + 1 < parts.size(); i += 2) {
    ...
    lexer::Token op{};
    if (const auto* op_tok = sdt::findChild(*op_node, parser::NodeType::TokenNode)) {
    ...
    if (result != nullptr && rhs != nullptr) {
      result = std::make_unique<ast::BinOpNode>(std::move(result), op, std::move(rhs));
    }
  }
  return result;
}
...
Ptr<ast::ExprNode> buildSimpleExpressionLocal(SDTBuilder& builder,
  ...
  if (!children.empty() && children.front()->type() == parser::NodeType::TokenNode) {
    if (auto tok = sdt::tokenOf(*children.front())) {
...
Ptr<ast::ExprNode> buildExpressionLocal(SDTBuilder& builder,
  ...
  if (left != nullptr && right != nullptr) {
    return std::make_unique<ast::BinOpNode>(std::move(left), op, std::move(right));
  }
  return left;
}
    ...
    if (auto tok = sdt::tokenOf(*child)) {
      ...
      if (index_list != nullptr) {
        ...
        for (const auto& part : child->children()) {
          if (auto tok = sdt::tokenOf(*part)) {
            if (tok->type == TokenType::IDENT && current != nullptr) {
              current = std::make_unique<ast::RecordAccessNode>(std::move(current),
                                                                *tok);
            }
          }
        }
...
Ptr<ast::ExprNode> SDTBuilder::buildVariableAccess(const parser::ParseNode& node) {
...
Ptr<ast::IfNode> SDTBuilder::buildIf(const parser::ParseNode& node) {
  ...
  return std::make_unique<ast::IfNode>(std::move(condition), std::move(then_branch),
...
Ptr<ast::WhileNode> SDTBuilder::buildWhile(const parser::ParseNode& node) {
  ...
  return std::make_unique<ast::WhileNode>(std::move(condition), std::move(compound));
}
...
Ptr<ast::ProcCallNode> SDTBuilder::buildProcCall(const parser::ParseNode& node) {
  lexer::Token name{};
  std::vector<Ptr<ast::ExprNode>> args;
