...
void SemanticAnalyzer::visit(ast::ForNode &node) {
  ...
  if (!entry) {
    std::cerr << "[Semantic Error] Undeclared iterator variable: "
              << node.iterator.lexeme << "\n";
  } else {
    // Validasi ekstra: Iterator WAJIB bertipe Integer (Ordinal)
    int int_type = get_base_type("integer");
    if (entry->type != int_type) {
      std::cerr << "[Semantic Error] Tipe iterator pada FOR loop wajib bertipe "
                   "Integer.\n";
    }
  }
  ...
  if (entry &&
      !isAssignmentCompatible(entry->type, node.initial->expression_type)) {
    std::cerr << "[Semantic Error] Nilai awal iterasi tidak "
                 "assignment-compatible dengan tipe iterator.\n";
  }
  if (entry &&
      !isAssignmentCompatible(entry->type, node.final->expression_type)) {
    std::cerr << "[Semantic Error] Nilai akhir iterasi tidak "
                 "assignment-compatible dengan tipe iterator.\n";
  }
...
void SemanticAnalyzer::visit(ast::FuncCallNode &node) {
  ...
  if (proc_name == "writeln" || proc_name == "readln" || proc_name == "write" ||
      proc_name == "read") {
  ...
  if (!entry) {
    ...
    std::cerr << "[Semantic Error] Identifier '" << node.id.lexeme
              << "' is not a procedure.\n";
  }
